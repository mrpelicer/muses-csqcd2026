...
class CalculationEngineApi():
    ...
    def read_yaml_file(self, uuid, path):
        url = f'''{self.conf['url_base']}/ce/download/{uuid}/{path.strip('/')}'''
    ...
    def download_job_files(self, job_id, output_dir='./downloads/', download_partial=False):

        job_status = self.job_list(uuid=job_id)
        if job_status['status'] == 'SUCCESS' or (job_status['status'] == 'STARTED' and download_partial):
            ...
            for file_info in job_status['files']:
                ...
                if file_info['size'] > 0:
                    self.download_job_file(job_id, file_info['path'], root_dir=output_dir)
            ...
            print(f'Job is still running. Status: {job_status["status"]}')
    ...
    def rate_limiter(self, response):
        ...
        if response.status_code == rate_limit_status_code:
            ...
            time.sleep(1.0 / self.conf['api_rate_limit'])
